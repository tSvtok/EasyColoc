<nav x-data="{ open: false }" class="bg-gray-800 border-b border-gray-700">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('dashboard') }}" class="text-xl font-bold text-indigo-400">EasyColoc</a>
                </div>
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if(Auth::user()->activeColocation())
                        <x-nav-link :href="route('colocations.show', Auth::user()->activeColocation())" :active="request()->routeIs('colocations.*')">
                            Ma Colocation
                        </x-nav-link>
                    @endif
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if(Auth::user()->activeColocation())
                <x-responsive-nav-link :href="route('colocations.show', Auth::user()->activeColocation())" :active="request()->routeIs('colocations.*')">
                    Ma Colocation
                </x-responsive-nav-link>
            @endif
